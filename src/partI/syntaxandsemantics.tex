...
Definite intagration is achieved via the \code{defInt()} transformation. \code{int(f, x, a, b)} is equivalent to \code{int(f, x)(b) - int(f, x)(a)} or, mathematically:

\begin{equation}
\int_a^b f(x) \text{d}x = \left( \int f(x) \text{d}x \right)(b) - \left( \int f(x) \text{d}x \right)(a)
\end{equation}

\subsection{$x$-Resolving}

$x$-resolving is the process by which integrations are solved. Consider an integral of type:

\begin{equation}
\intx{\frac{1}{x}}
\end{equation}

To solve this integral, traditional approach is to subsitute $\ln|x| = u$, and then differentiating both sides with respect to x to find $\frac{1}{x} \text{d}x = \text{d}u$, which is the inside of the integral, hence the solution is found.\\

$x$-resolving is finding an expression, $u$, such that, the entire inside of the integral expression can be subsituted for $d{u}$, when this constraint is fulfilled, the $u + c$ is said to be the solution of the integral.\\

$x$-resolving is done, simply by checking if the inside of the integral matches any of differential transformation results, and determining the $u$. Moreover, while resolving for $x$, once can divide integral sums, or unify them.

\begin{equation}
\intx{u} + \intx{v} \Leftrightarrow \intx{u + v}
\end{equation}
